    ...
    useEffect(() => {
        if (cidades.length > 0) {
            setOptions(
                cidades?.map((cidade) => {
                    return {
                        value: cidade.NM_CIDADE.trim(),
                        label: cidade.NM_CIDADE.trim(),
                    };
                })
            );
        }
    ...
    useEffect(() => {
        ...
        if (props.value) {
            ...
            for (let [index, cidade] of Object.entries(todasCidades)) {
                if (cidade.value.toUpperCase() == props.value.toUpperCase()) {
    ...
    const getCidades = async () => {
        ...
        try {
            const response = await axiosInstance.get(
                `cidades/cidades-por-estado/${props.cdUF}/${props.cdPais || 1}`
            );
